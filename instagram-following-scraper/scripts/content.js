...
async function DoScrape(){
	...
	notifer_el.innerHTML = "Instagram Target Page Following Scraper (by v-User) </ br> Scraping process started...";
	...
	if (loading_el && memberRows.length > 1) {
		...
		await doLoading();
		console.log("All Following Are loaded");
		...
		notifer_el.innerHTML = `Instagram Target Page Following Scraper (by v-User) </ br> Scraping process finished and found ${uniqueCounter} unique user.`;
		...
		notifer_el.innerHTML = "Instagram Target Page Following Scraper (by v-User) </ br> Error: Members list is not visible!";
